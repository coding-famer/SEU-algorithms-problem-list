...
int main()
{
    ...
    for (int x = 0;x < M;x++)
    {
        ...
        int N = n*5/12;
        int* re=new int[N];
        for(int i=rand()%n;count<n;i = rand() % n)
        {
            if (flag[i] == 1)
                continue;
            if (count < N)
        ...
        {
            int tempd = re[i];
            answer answ = ans[tempd];
            for (int j = 0;j < answ.t.size();j++)
            {
                if (j % 2 == 1)
                {
                    int z = answ.t[j];
        ...
        for (int i = 0;i < min(N,n);i++)
        {
            ...
            int tempd = re[i];
            int pre[401];
            Dijkstra(m, req[tempd].s, req[tempd].t, pre, g, e, adjust);

            int init = req[tempd].s;
            int fina = req[tempd].t;
            int temp = fina;
            int r[401];
